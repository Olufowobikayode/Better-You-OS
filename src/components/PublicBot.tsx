...
import { Globe, ArrowDownToLine, Cpu, ArrowUpFromLine } from 'lucide-react';

export default function PublicBot() {
  const { processLogs, isEngineRunning } = useAppContext();
  const logsEndRef = useRef<HTMLDivElement>(null);

  const publicLogs = processLogs.filter(log => log.bot === 'public');
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'send': return <ArrowUpFromLine className="w-4 h-4 text-emerald-400" />;
    ...
    <div className="flex flex-col h-full max-w-4xl mx-auto bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 bg-gray-950 border-b border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Public Bot Process Monitor</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#0a0a0a]">
        {publicLogs.length === 0 ? (
          <div className="text-center text-gray-500 mt-20">
            ...
            <p className="text-sm mt-2">Start the AI Engine and send a message to your Public Bot on Telegram.</p>
          </div>
        ) : (
          publicLogs.map(log => (
            <div key={log.id} className="flex items-start gap-3 p-3 rounded-lg bg-gray-900 border border-gray-800 font-mono text-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`whitespace-pre-wrap break-words ${
                  ...
                  log.type === 'send' ? 'text-emerald-200' :
                  'text-gray-300'
                }`}>
                  {log.content}
                </p>
